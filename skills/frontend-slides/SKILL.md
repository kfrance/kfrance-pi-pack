...
**Ask all of the following questions together** so the user can answer at once:
...
4. **Inline Editing** — Do you need to edit text directly in the browser after generation? (Yes — recommended, allows in-browser editing with auto-save / No — keeps file smaller)
...
5. **Confirm with user:** "Does this slide outline and image selection look right?" (Looks good / Adjust images / Adjust outline)
...
Ask the user to pick 1-2 moods:
...
Save previews to `.slide-previews/` (style-a.html, style-b.html, style-c.html). Each should be self-contained, ~50-100 lines, showing one animated title slide.

Open each preview automatically for the user (use `xdg-open` on Linux, `open` on macOS).
...
1. **Extract content** — Run `uv run scripts/extract-pptx.py <input.pptx> <output_dir>` (the script uses python-pptx; uv handles dependencies)
...
2. **Open** — Launch the HTML file in the browser (`xdg-open` on Linux, `open` on macOS)
